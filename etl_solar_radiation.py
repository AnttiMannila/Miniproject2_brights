...
def concat_venue_and_radiation():
    venue_path = "C:\\project2\\Miniproject2_brights\\venue"
    venue_files = glob.glob(os.path.join(venue_path, "*.csv"))
    radiation = pd.read_csv('direct_solar_radiation_2004_2024_values.csv')
    venue = pd.read_csv('vihti.csv')
    ...
    for file in venue_files:
        ...
        # df['Cloud cover [1/8]'] = df['Cloud cover[1/8]'].str.extract('(\d+)').astype(float)
        dfs.append(df)
    ...
    venue.to_csv('venue.csv')
    
    
if __name__ == '__main__':
    radiation_to_one_file()
    # concat_venue_and_radiation()